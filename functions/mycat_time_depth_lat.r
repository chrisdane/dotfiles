...
if (length(args) == 0 ||
    ...
    if (T) { 
        ...
        # lat,depth,time does not work
        # time,depth,lat works but repeats time
        message("reorder dimensions ...")
        for (i in 1:length(fs)) {
            cmd <- paste0("ncpdq -a time,depth,lat ",
    ...
    if (T) {
        # without does not work
        # time,depth,lat works for ncview; output ncdump : variable(time, depth, lat)
        message("reorder dimensions ...")
        cmd <- paste0("ncpdq -O -a time,depth,lat ",
                      dirname(outname), "/", basename(outname), " ",
                      dirname(outname), "/", basename(outname))
        cat(paste0(cmd, " ..."), "\n")
        system(cmd)
