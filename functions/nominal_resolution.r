...
for (fi in seq_along(fs)) {
    ...
    cmd <- paste0(cdo, " -s -griddx -seltimestep,1 ", select, " ", foutselect, " ", foutx)
    message("run `", cmd, "` ...")
    check <- system(cmd)
    if (check != 0) stop("error")
    ...
    cmd <- paste0(cdo, " -s -griddy -seltimestep,1 ", select, " ", foutselect, " ", fouty)
    message("run `", cmd, "` ...")
    check <- system(cmd)
    if (check != 0) stop("error")
    ...
    invisible(file.remove(foutx, fouty, foutxy, foutdmax))
